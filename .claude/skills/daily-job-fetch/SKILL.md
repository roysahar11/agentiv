...
### Manual Mode (default)
- **Sources**: All — LinkedIn IL + Intl, WhatsApp groups, web scrapers, Chrome sources (AllJobs, Built In, Wellfound).
...
4. **Max applicants**: from `## Filtering` → `Max applicants`. Default: `30` if not present.

These values are used throughout the pipeline:
- If international is `false`: skip international scrapers, LinkedIn Intl, international pipeline in Steps 1–5.
...
Where `{LINKEDIN_SCOPES}` = "Israel + International" if international is enabled, "Israel only" otherwise.
...
Spawn `job-description-fetcher` agents (foreground, `general-purpose` with `model="sonnet"`). Run IL first, then Intl if enabled (they share Chrome).
...
Max applicants threshold: {MAX_APPLICANTS}
...
Read profile/context.md for a summary of the user's background, skills, and career stage.
...
Read profile/context.md for a summary of the user's background, skills, and career stage. Check config/user.md for citizenship/work authorization details.
...
Every job entry in the report — Relevant, Discuss, AND Skipped — must include a URL or source link. The user needs links to assess relevance themselves. If a job came from WhatsApp without a URL, include the WhatsApp group name and any context for manual search.

#### Step 4b: Report Errors

If any source encountered errors, include them in the report and send a WhatsApp alert via `/whatsapp autonomous send-message` (phone from `config/user.md`).
...
After quick-apply agents finish, update the report with draft statuses and errors from quick-apply, then send to the user's self-chat via `/whatsapp autonomous send-message` (phone from `config/user.md`). This is the primary deliverable — always sent. It is distinct from the per-job WhatsApp notifications that quick-apply agents send.
